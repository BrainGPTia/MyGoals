// ========================================
// DONNÉES ET ÉTAT
// ========================================

// S'assure que les IDs stockés sont des strings, ce qui est le cas avec Date.now().toString()
let goals = JSON.parse(localStorage.getItem('goals')) || [];
let checkins = JSON.parse(localStorage.getItem('checkins')) || [];
// Utilisation d'un objet pour les interactions sociales, la clé est l'ID de l'objectif
let socialInteractions = JSON.parse(localStorage.getItem('socialInteractions')) || {};

// ========================================
// UTILITAIRES (Déplacé en haut pour une meilleure accessibilité)
// ========================================

function saveGoals() {
    localStorage.setItem('goals', JSON.stringify(goals));
}

function saveCheckIns() {
    localStorage.setItem('checkins', JSON.stringify(checkins));
}

function saveSocialInteractions() {
    localStorage.setItem('socialInteractions', JSON.stringify(socialInteractions));
}

function formatDate(dateString) {
    const date = new Date(dateString);
    return date.toLocaleDateString('fr-FR', { 
        day: '2-digit', 
        month: '2-digit', 
        year: 'numeric',
        hour: '2-digit',
        minute: '2-digit'
    });
}

function getDisplayedName(goal) {
    // Correction: Assurez-vous que les valeurs correspondent aux attributs 'value' de votre HTML/futur HTML
    if (goal.displayName === 'anonyme') return 'Anonyme';
    // Utilise userName ou un fallback si le champ est vide
    if (goal.displayName === 'pseudo') return goal.userName || 'Utilisateur Anonyme'; 
    if (goal.displayName === 'nom') return goal.userName || 'Utilisateur'; // Assurez-vous que l'affichage est cohérent
    return 'Utilisateur';
}

// ========================================
// NAVIGATION
// ========================================

document.querySelectorAll('.nav-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        const section = btn.dataset.section;
        
        // Update active nav button
        document.querySelectorAll('.nav-btn').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        
        // Update active section
        document.querySelectorAll('.section').forEach(s => s.classList.remove('active'));
        const activeSection = document.getElementById(section);
        if (activeSection) activeSection.classList.add('active');
        
        // Render appropriate section
        if (section === 'mes-objectifs') renderMyGoals();
        if (section === 'objectifs-publics') renderPublicGoals();
        if (section === 'check-in') renderCheckIn();
    });
});

// Assure que la bonne section est chargée et rendue au démarrage (simule un clic sur le premier bouton)
document.addEventListener('DOMContentLoaded', () => {
    // Trouve le bouton actif ou le premier bouton de navigation et simule un clic
    const initialBtn = document.querySelector('.nav-btn.active') || document.querySelector('.nav-btn');
    if (initialBtn) {
        initialBtn.click();
    } else {
        // Fallback si aucun bouton n'est trouvé ou rendu
        renderMyGoals();
        const defaultSection = document.getElementById('mes-objectifs');
        if (defaultSection) defaultSection.classList.add('active');
    }
});


// ========================================
// MODAL GESTION
// ========================================

const modalGoal = document.getElementById('modal-goal');
const modalDetails = document.getElementById('modal-goal-details');
const goalProgressSlider = document.getElementById('goal-progress');
const goalProgressValue = document.getElementById('goal-progress-value');
const checkinProgressSlider = document.getElementById('checkin-progress');
const checkinProgressValue = document.getElementById('checkin-progress-value');


if (document.getElementById('btn-new-goal')) {
    document.getElementById('btn-new-goal').addEventListener('click', () => {
        openGoalModal();
    });
}

if (document.getElementById('modal-goal-close')) {
    document.getElementById('modal-goal-close').addEventListener('click', () => {
        modalGoal.classList.remove('active');
    });
}

if (document.getElementById('modal-details-close')) {
    document.getElementById('modal-details-close').addEventListener('click', () => {
        modalDetails.classList.remove('active');
    });
}


// ========================================
// PROGRESS SLIDERS
// ========================================

if (goalProgressSlider) {
    goalProgressSlider.addEventListener('input', (e) => {
        if (goalProgressValue) goalProgressValue.textContent = e.target.value + '%';
    });
}

if (checkinProgressSlider) {
    checkinProgressSlider.addEventListener('input', (e) => {
        if (checkinProgressValue) checkinProgressValue.textContent = e.target.value + '%';
    });
}


// ========================================
// CRÉER/MODIFIER OBJECTIF
// ========================================

function openGoalModal(goalId = null) {
    const form = document.getElementById('goal-form');
    if (!form || !modalGoal || !goalProgressValue) return;

    form.reset();
    document.getElementById('goal-id').value = ''; // Réinitialise l'ID

    if (goalId) {
        const goal = goals.find(g => g.id === goalId);
        if (goal) {
            document.getElementById('modal-goal-title').textContent = 'Modifier l\'objectif';
            document.getElementById('goal-id').value = goal.id;
            document.getElementById('goal-title').value = goal.title;
            document.getElementById('goal-description').value = goal.description;
            
            // Assurez-vous que l'élément radio existe avant de le cocher
            const typeRadio = document.querySelector(`input[name="goal-type"][value="${goal.type}"]`);
            if (typeRadio) typeRadio.checked = true;

            document.getElementById('goal-progress').value = goal.progress;
            goalProgressValue.textContent = goal.progress + '%';

            const visibilityRadio = document.querySelector(`input[name="goal-visibility"][value="${goal.visibility}"]`);
            if (visibilityRadio) visibilityRadio.checked = true;

            const displayNameRadio = document.querySelector(`input[name="goal-display-name"][value="${goal.displayName}"]`);
            if (displayNameRadio) displayNameRadio.checked = true;

            document.getElementById('goal-user-name').value = goal.userName || '';
            document.getElementById('goal-allow-comments').checked = goal.allowComments;
        }
    } else {
        document.getElementById('modal-goal-title').textContent = 'Créer un objectif';
        goalProgressValue.textContent = '0%';
    }
    
    modalGoal.classList.add('active');
}

if (document.getElementById('goal-form')) {
    document.getElementById('goal-form').addEventListener('submit', (e) => {
        e.preventDefault();
        
        const goalId = document.getElementById('goal-id').value;
        const title = document.getElementById('goal-title').value.trim();
        const description = document.getElementById('goal-description').value.trim();
        
        // Vérification des champs requis
        if (!title) {
            alert('Le titre de l\'objectif est requis.');
            return;
        }

        const typeElement = document.querySelector('input[name="goal-type"]:checked');
        const visibilityElement = document.querySelector('input[name="goal-visibility"]:checked');
        const displayNameElement = document.querySelector('input[name="goal-display-name"]:checked');

        // Récupérer les valeurs ou utiliser des valeurs par défaut si non trouvées (erreur dans le HTML ?)
        const type = typeElement ? typeElement.value : 'personnel'; 
        const progress = parseInt(document.getElementById('goal-progress').value) || 0;
        const visibility = visibilityElement ? visibilityElement.value : 'privé';
        const displayName = displayNameElement ? displayNameElement.value : 'anonyme';
        const userName = document.getElementById('goal-user-name').value.trim();
        const allowComments = document.getElementById('goal-allow-comments').checked;
        
        if (goalId) {
            // Modifier
            const index = goals.findIndex(g => g.id === goalId);
            if (index !== -1) {
                goals[index] = {
                    ...goals[index],
                    title,
                    description,
                    type,
                    progress,
                    visibility,
                    displayName,
                    userName,
                    allowComments,
                    lastUpdate: new Date().toISOString()
                };
            }
        } else {
            // Créer
            const newGoal = {
                id: Date.now().toString(), // Maintient l'ID comme string
                title,
                description,
                type,
                progress,
                visibility,
                displayName,
                userName,
                allowComments,
                createdAt: new Date().toISOString(),
                lastUpdate: new Date().toISOString()
            };
            goals.push(newGoal);
        }
        
        saveGoals();
        if (modalGoal) modalGoal.classList.remove('active');
        renderMyGoals();
    });
}

// ========================================
// AFFICHAGE MES OBJECTIFS
// ========================================

function renderMyGoals() {
    const container = document.getElementById('my-goals-list');
    if (!container) return; // Garde-fou

    if (goals.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <i class="fas fa-bullseye"></i>
                <p>Aucun objectif créé</p>
                <p style="font-size: 14px; margin-top: 8px;">Cliquez sur "Créer un objectif" pour commencer</p>
            </div>
        `;
        return;
    }
    
    container.innerHTML = goals.map(goal => `
        <div class="goal-item" data-id="${goal.id}">
            <div class="goal-header">
                <div>
                    <div class="goal-title">${goal.title}</div>
                    <span class="goal-type">${goal.type}</span>
                </div>
            </div>
            
            ${goal.description ? `<div class="goal-description">${goal.description}</div>` : ''}
            
            <div class="progress-container">
                <div class="progress-label">
                    <span>Progression</span>
                    <span>${goal.progress}%</span>
                </div>
                <div class="progress-bar">
                    <div class="progress-fill" style="width: ${goal.progress}%"></div>
                </div>
            </div>
            
            <div class="goal-meta">
                <span><i class="fas fa-eye${goal.visibility === 'privé' ? '-slash' : ''}"></i> ${goal.visibility === 'privé' ? 'Privé' : 'Public'}</span>
                <span><i class="fas fa-calendar"></i> Créé le ${formatDate(goal.createdAt)}</span>
                <span><i class="fas fa-clock"></i> Mis à jour le ${formatDate(goal.lastUpdate)}</span>
            </div>
            
            <div class="goal-actions">
                <button class="btn btn-secondary" onclick="openGoalModal('${goal.id}')">
                    <i class="fas fa-edit"></i> Modifier
                </button>
                <button class="btn btn-secondary" onclick="viewGoalDetails('${goal.id}')">
                    <i class="fas fa-info-circle"></i> Détails
                </button>
                <button class="btn btn-danger" onclick="deleteGoal('${goal.id}')">
                    <i class="fas fa-trash"></i> Supprimer
                </button>
                ${goal.progress === 100 ? '<button class="btn btn-success"><i class="fas fa-check"></i> Terminé</button>' : ''}
            </div>
        </div>
    `).join('');
}

// ========================================
// AFFICHAGE OBJECTIFS PUBLICS
// ========================================

function renderPublicGoals() {
    const container = document.getElementById('public-goals-list');
    if (!container) return;

    const publicGoals = goals.filter(g => g.visibility === 'public');
    
    if (publicGoals.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <i class="fas fa-globe"></i>
                <p>Aucun objectif public</p>
                <p style="font-size: 14px; margin-top: 8px;">Les objectifs publics apparaîtront ici</p>
            </div>
        `;
        return;
    }
    
    container.innerHTML = publicGoals.map(goal => {
        const displayedName = getDisplayedName(goal);
        
        // Assurez-vous que l'entrée existe pour cet objectif dans socialInteractions
        if (!socialInteractions[goal.id]) {
            socialInteractions[goal.id] = { likes: [], dislikes: [], comments: [] };
        }

        const interactions = socialInteractions[goal.id];
        // Note: 'current-user' est un placeholder, remplacez-le par un ID utilisateur réel si l'application avait une gestion d'utilisateurs.
        const currentUserIdentifier = 'current-user'; 
        const userLiked = interactions.likes.includes(currentUserIdentifier);
        const userDisliked = interactions.dislikes.includes(currentUserIdentifier);
        
        return `
            <div class="goal-item" data-id="${goal.id}">
                <div class="goal-header">
                    <div>
                        <div class="goal-title">${goal.title}</div>
                        <span class="goal-type">${goal.type}</span>
                    </div>
                </div>
                
                ${goal.description ? `<div class="goal-description">${goal.description}</div>` : ''}
                
                <div class="progress-container">
                    <div class="progress-label">
                        <span>Progression</span>
                        <span>${goal.progress}%</span>
                    </div>
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: ${goal.progress}%"></div>
                    </div>
                </div>
                
                <div class="goal-meta">
                    <span><i class="fas fa-user"></i> ${displayedName}</span>
                    <span><i class="fas fa-clock"></i> Mis à jour le ${formatDate(goal.lastUpdate)}</span>
                </div>
                
                <div class="social-actions">
                    <button class="social-btn ${userLiked ? 'active' : ''}" onclick="toggleLike('${goal.id}', '${currentUserIdentifier}')">
                        <i class="fas fa-thumbs-up"></i>
                        <span>${interactions.likes.length}</span>
                    </button>
                    <button class="social-btn dislike ${userDisliked ? 'active' : ''}" onclick="toggleDislike('${goal.id}', '${currentUserIdentifier}')">
                        <i class="fas fa-thumbs-down"></i>
                        <span>${interactions.dislikes.length}</span>
                    </button>
                    ${goal.allowComments ? `
                        <button class="social-btn" onclick="toggleComments('${goal.id}')">
                            <i class="fas fa-comment"></i>
                            <span>${interactions.comments.length}</span>
                        </button>
                    ` : ''}
                </div>
                
                ${goal.allowComments ? `
                    <div class="comments-section" id="comments-${goal.id}" style="display: none;">
                        <div class="comment-form">
                            <input type="text" placeholder="Écrire un commentaire..." id="comment-input-${goal.id}">
                            <button class="btn" onclick="addComment('${goal.id}', '${currentUserIdentifier}')">
                                <i class="fas fa-paper-plane"></i>
                            </button>
                        </div>
                        <div id="comments-list-${goal.id}">
                            ${interactions.comments.map(c => `
                                <div class="comment-item">
                                    <div class="comment-author">${c.author}</div>
                                    <div>${c.text}</div>
                                </div>
                            `).join('')}
                        </div>
                    </div>
                ` : ''}
            </div>
        `;
    }).join('');
}

// ========================================
// INTERACTIONS SOCIALES
// ========================================

function toggleLike(goalId, userId) {
    if (!socialInteractions[goalId]) {
        socialInteractions[goalId] = { likes: [], dislikes: [], comments: [] };
    }
    
    const likes = socialInteractions[goalId].likes;
    const dislikes = socialInteractions[goalId].dislikes;
    const userIndex = likes.indexOf(userId);
    
    if (userIndex > -1) {
        likes.splice(userIndex, 1);
    } else {
        likes.push(userId);
        // Retirer le dislike si l'utilisateur a liké
        const dislikeIndex = dislikes.indexOf(userId);
        if (dislikeIndex > -1) dislikes.splice(dislikeIndex, 1);
    }
    
    saveSocialInteractions();
    renderPublicGoals();
}

function toggleDislike(goalId, userId) {
    if (!socialInteractions[goalId]) {
        socialInteractions[goalId] = { likes: [], dislikes: [], comments: [] };
    }
    
    const likes = socialInteractions[goalId].likes;
    const dislikes = socialInteractions[goalId].dislikes;
    const userIndex = dislikes.indexOf(userId);
    
    if (userIndex > -1) {
        dislikes.splice(userIndex, 1);
    } else {
        dislikes.push(userId);
        // Retirer le like si l'utilisateur a disliké
        const likeIndex = likes.indexOf(userId);
        if (likeIndex > -1) likes.splice(likeIndex, 1);
    }
    
    saveSocialInteractions();
    renderPublicGoals();
}

function toggleComments(goalId) {
    const commentsSection = document.getElementById(`comments-${goalId}`);
    if (commentsSection) {
        // Toggle l'affichage
        commentsSection.style.display = commentsSection.style.display === 'none' ? 'block' : 'none';
        
        // Si l'affichage est activé, assurez-vous de scroll vers l'objectif
        if (commentsSection.style.display === 'block') {
             // Remonte l'affichage pour l'objectif courant
            commentsSection.closest('.goal-item').scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
    }
}

function addComment(goalId, userId) {
    const input = document.getElementById(`comment-input-${goalId}`);
    if (!input) return;

    const text = input.value.trim();
    
    if (!text) {
        alert('Le commentaire ne peut pas être vide.');
        return;
    }
    
    if (!socialInteractions[goalId]) {
        socialInteractions[goalId] = { likes: [], dislikes: [], comments: [] };
    }

    // Détermine le nom de l'auteur. Pour l'exemple, nous utilisons 'Utilisateur'
    const authorName = userId === 'current-user' ? 'Moi' : 'Utilisateur'; 
    
    socialInteractions[goalId].comments.push({
        author: authorName,
        text: text,
        date: new Date().toISOString()
    });
    
    saveSocialInteractions();
    input.value = '';
    renderPublicGoals();
    // Le re-render ferme la section, il faut la rouvrir et scroller jusqu'à elle.
    setTimeout(() => {
        toggleComments(goalId); // Ouvre la section après le re-render
    }, 50); // Petit délai pour laisser le temps au DOM de se mettre à jour
}

// ========================================
// CHECK-IN QUOTIDIEN
// ========================================

function renderCheckIn() {
    const select = document.getElementById('checkin-goal-select');
    if (!select || !checkinProgressSlider || !checkinProgressValue) return;

    // Réinitialisation de la sélection et du slider
    select.innerHTML = '<option value="">-- Sélectionner un objectif --</option>' + 
        goals.map(g => `<option value="${g.id}">${g.title}</option>`).join('');
    
    checkinProgressSlider.value = 0;
    checkinProgressValue.textContent = '0%';
    
    // Ajout d'un écouteur d'événement unique pour la sélection
    // Correction: On doit retirer l'ancien listener pour éviter les duplications
    select.onchange = (e) => { 
        const goal = goals.find(g => g.id === e.target.value);
        if (goal) {
            checkinProgressSlider.value = goal.progress;
            checkinProgressValue.textContent = goal.progress + '%';
        } else {
            // Si rien n'est sélectionné
            checkinProgressSlider.value = 0;
            checkinProgressValue.textContent = '0%';
        }
    };
    
    renderCheckInHistory();
}

if (document.getElementById('btn-save-checkin')) {
    document.getElementById('btn-save-checkin').addEventListener('click', () => {
        const goalId = document.getElementById('checkin-goal-select').value;
        const note = document.getElementById('checkin-note').value.trim();
        const progress = parseInt(document.getElementById('checkin-progress').value);
        
        if (!goalId) {
            alert('Veuillez sélectionner un objectif');
            return;
        }
        
        // La note n'est pas strictement obligatoire pour tous les check-ins, mais selon votre validation:
        // if (!note) {
        //     alert('Veuillez écrire une note');
        //     return;
        // }
        
        // Mettre à jour la progression de l'objectif
        const goal = goals.find(g => g.id === goalId);
        if (goal) {
            goal.progress = progress;
            goal.lastUpdate = new Date().toISOString();
            saveGoals();
            
            // Enregistrer le check-in
            const checkin = {
                id: Date.now().toString(),
                goalId: goalId,
                goalTitle: goal.title, // Stocke le titre pour l'historique
                note: note,
                progress: progress,
                date: new Date().toISOString()
            };
            
            checkins.push(checkin);
            saveCheckIns();
            
            // Réinitialiser le formulaire
            document.getElementById('checkin-goal-select').value = '';
            document.getElementById('checkin-note').value = '';
            checkinProgressSlider.value = 0;
            checkinProgressValue.textContent = '0%';

            alert('Check-in enregistré avec succès !');
            
            // Re-rendre la liste des objectifs personnels si nous sommes sur cette section
            const myGoalsSection = document.getElementById('mes-objectifs');
            if (myGoalsSection && myGoalsSection.classList.contains('active')) {
                renderMyGoals();
            }
            
            renderCheckInHistory();
        } else {
             alert('Objectif non trouvé.');
        }
    });
}


function renderCheckInHistory() {
    const container = document.getElementById('checkin-history');
    if (!container) return;
    
    if (checkins.length === 0) {
        container.innerHTML = `
            <div class="card">
                <div class="empty-state">
                    <i class="fas fa-clipboard-list"></i>
                    <p>Aucun check-in enregistré</p>
                </div>
            </div>
        `;
        return;
    }
    
    const sortedCheckins = [...checkins].sort((a, b) => new Date(b.date) - new Date(a.date));
    
    container.innerHTML = `
        <div class="card">
            <h3 style="margin-bottom: 20px;">Historique des check-ins</h3>
            ${sortedCheckins.map(c => `
                <div class="checkin-item">
                    <div class="checkin-date">${formatDate(c.date)}</div>
                    <div class="checkin-progress">
                        <strong>${c.goalTitle}</strong> - ${c.progress}%
                    </div>
                    <div class="checkin-note">${c.note || 'Aucune note'}</div>
                </div>
            `).join('')}
        </div>
    `;
}

// ========================================
// DÉTAILS OBJECTIF
// ========================================

function viewGoalDetails(goalId) {
    const goal = goals.find(g => g.id === goalId);
    if (!goal || !modalDetails) return;
    
    const goalCheckins = checkins.filter(c => c.goalId === goalId);
    
    const content = `
        <div style="padding-right: 15px;">
            <h3 style="margin-bottom: 16px;">${goal.title}</h3>
            <div class="goal-meta" style="margin-bottom: 20px;">
                <span><i class="fas fa-tag"></i> ${goal.type}</span>
                <span><i class="fas fa-eye${goal.visibility === 'privé' ? '-slash' : ''}"></i> ${goal.visibility}</span>
                <span><i class="fas fa-calendar"></i> Créé le ${formatDate(goal.createdAt)}</span>
                <span><i class="fas fa-clock"></i> Dernière MAJ: ${formatDate(goal.lastUpdate)}</span>
            </div>
            
            ${goal.description ? `<p style="margin-bottom: 20px; color: var(--text-light);">${goal.description}</p>` : ''}
            
            <div class="progress-container">
                <div class="progress-label">
                    <span>Progression actuelle</span>
                    <span>${goal.progress}%</span>
                </div>
                <div class="progress-bar">
                    <div class="progress-fill" style="width: ${goal.progress}%"></div>
                </div>
            </div>
            
            <div style="margin-top: 24px;">
                <h4 style="margin-bottom: 12px;">Check-ins (${goalCheckins.length})</h4>
                ${goalCheckins.length === 0 ? '<p style="color: var(--text-light);">Aucun check-in pour cet objectif</p>' : 
                    goalCheckins.sort((a, b) => new Date(b.date) - new Date(a.date)).map(c => `
                        <div class="checkin-item">
                            <div class="checkin-date">${formatDate(c.date)}</div>
                            <div class="checkin-progress">${c.progress}%</div>
                            <div class="checkin-note">${c.note || 'Aucune note'}</div>
                        </div>
                    `).join('')
                }
            </div>
        </div>
    `;
    
    const goalDetailsContent = document.getElementById('goal-details-content');
    if (goalDetailsContent) {
        goalDetailsContent.innerHTML = content;
        modalDetails.classList.add('active');
    }
}

// ========================================
// SUPPRESSION OBJECTIF
// ========================================

function deleteGoal(goalId) {
    if (!confirm('Êtes-vous sûr de vouloir supprimer cet objectif ? Cette action est irréversible et supprimera également les check-ins associés.')) return;
    
    goals = goals.filter(g => g.id !== goalId);
    checkins = checkins.filter(c => c.goalId !== goalId);
    // Suppression des interactions sociales
    if (socialInteractions[goalId]) {
        delete socialInteractions[goalId];
    }
    
    saveGoals();
    saveCheckIns();
    saveSocialInteractions();
    renderMyGoals();
}

// ========================================
// INITIALISATION
// ========================================

// La fonction d'initialisation a été déplacée dans le listener DOMContentLoaded pour s'assurer que le DOM est prêt.
// renderMyGoals(); // Supprimé, géré par DOMContentLoadedconst modalDetails = document.getElementById('modal-goal-details');

document.getElementById('btn-new-goal').addEventListener('click', () => {
    openGoalModal();
});

document.getElementById('modal-goal-close').addEventListener('click', () => {
    modalGoal.classList.remove('active');
});

document.getElementById('modal-details-close').addEventListener('click', () => {
    modalDetails.classList.remove('active');
});

// ========================================
// PROGRESS SLIDERS
// ========================================

const goalProgressSlider = document.getElementById('goal-progress');
const goalProgressValue = document.getElementById('goal-progress-value');

goalProgressSlider.addEventListener('input', (e) => {
    goalProgressValue.textContent = e.target.value + '%';
});

const checkinProgressSlider = document.getElementById('checkin-progress');
const checkinProgressValue = document.getElementById('checkin-progress-value');

checkinProgressSlider.addEventListener('input', (e) => {
    checkinProgressValue.textContent = e.target.value + '%';
});

// ========================================
// CRÉER/MODIFIER OBJECTIF
// ========================================

function openGoalModal(goalId = null) {
    const form = document.getElementById('goal-form');
    form.reset();
    
    if (goalId) {
        const goal = goals.find(g => g.id === goalId);
        if (goal) {
            document.getElementById('modal-goal-title').textContent = 'Modifier l\'objectif';
            document.getElementById('goal-id').value = goal.id;
            document.getElementById('goal-title').value = goal.title;
            document.getElementById('goal-description').value = goal.description;
            document.querySelector(`input[name="goal-type"][value="${goal.type}"]`).checked = true;
            document.getElementById('goal-progress').value = goal.progress;
            goalProgressValue.textContent = goal.progress + '%';
            document.querySelector(`input[name="goal-visibility"][value="${goal.visibility}"]`).checked = true;
            document.querySelector(`input[name="goal-display-name"][value="${goal.displayName}"]`).checked = true;
            document.getElementById('goal-user-name').value = goal.userName || '';
            document.getElementById('goal-allow-comments').checked = goal.allowComments;
        }
    } else {
        document.getElementById('modal-goal-title').textContent = 'Créer un objectif';
        goalProgressValue.textContent = '0%';
    }
    
    modalGoal.classList.add('active');
}

document.getElementById('goal-form').addEventListener('submit', (e) => {
    e.preventDefault();
    
    const goalId = document.getElementById('goal-id').value;
    const title = document.getElementById('goal-title').value;
    const description = document.getElementById('goal-description').value;
    const type = document.querySelector('input[name="goal-type"]:checked').value;
    const progress = parseInt(document.getElementById('goal-progress').value);
    const visibility = document.querySelector('input[name="goal-visibility"]:checked').value;
    const displayName = document.querySelector('input[name="goal-display-name"]:checked').value;
    const userName = document.getElementById('goal-user-name').value;
    const allowComments = document.getElementById('goal-allow-comments').checked;
    
    if (goalId) {
        // Modifier
        const index = goals.findIndex(g => g.id === goalId);
        if (index !== -1) {
            goals[index] = {
                ...goals[index],
                title,
                description,
                type,
                progress,
                visibility,
                displayName,
                userName,
                allowComments,
                lastUpdate: new Date().toISOString()
            };
        }
    } else {
        // Créer
        const newGoal = {
            id: Date.now().toString(),
            title,
            description,
            type,
            progress,
            visibility,
            displayName,
            userName,
            allowComments,
            createdAt: new Date().toISOString(),
            lastUpdate: new Date().toISOString()
        };
        goals.push(newGoal);
    }
    
    saveGoals();
    modalGoal.classList.remove('active');
    renderMyGoals();
});

// ========================================
// AFFICHAGE MES OBJECTIFS
// ========================================

function renderMyGoals() {
    const container = document.getElementById('my-goals-list');
    
    if (goals.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <i class="fas fa-bullseye"></i>
                <p>Aucun objectif créé</p>
                <p style="font-size: 14px; margin-top: 8px;">Cliquez sur "Créer un objectif" pour commencer</p>
            </div>
        `;
        return;
    }
    
    container.innerHTML = goals.map(goal => `
        <div class="goal-item">
            <div class="goal-header">
                <div>
                    <div class="goal-title">${goal.title}</div>
                    <span class="goal-type">${goal.type}</span>
                </div>
            </div>
            
            ${goal.description ? `<div class="goal-description">${goal.description}</div>` : ''}
            
            <div class="progress-container">
                <div class="progress-label">
                    <span>Progression</span>
                    <span>${goal.progress}%</span>
                </div>
                <div class="progress-bar">
                    <div class="progress-fill" style="width: ${goal.progress}%"></div>
                </div>
            </div>
            
            <div class="goal-meta">
                <span><i class="fas fa-eye${goal.visibility === 'privé' ? '-slash' : ''}"></i> ${goal.visibility === 'privé' ? 'Privé' : 'Public'}</span>
                <span><i class="fas fa-calendar"></i> Créé le ${formatDate(goal.createdAt)}</span>
                <span><i class="fas fa-clock"></i> Mis à jour le ${formatDate(goal.lastUpdate)}</span>
            </div>
            
            <div class="goal-actions">
                <button class="btn btn-secondary" onclick="openGoalModal('${goal.id}')">
                    <i class="fas fa-edit"></i> Modifier
                </button>
                <button class="btn btn-secondary" onclick="viewGoalDetails('${goal.id}')">
                    <i class="fas fa-info-circle"></i> Détails
                </button>
                <button class="btn btn-danger" onclick="deleteGoal('${goal.id}')">
                    <i class="fas fa-trash"></i> Supprimer
                </button>
                ${goal.progress === 100 ? '<button class="btn btn-success"><i class="fas fa-check"></i> Terminé</button>' : ''}
            </div>
        </div>
    `).join('');
}

// ========================================
// AFFICHAGE OBJECTIFS PUBLICS
// ========================================

function renderPublicGoals() {
    const container = document.getElementById('public-goals-list');
    const publicGoals = goals.filter(g => g.visibility === 'public');
    
    if (publicGoals.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <i class="fas fa-globe"></i>
                <p>Aucun objectif public</p>
                <p style="font-size: 14px; margin-top: 8px;">Les objectifs publics apparaîtront ici</p>
            </div>
        `;
        return;
    }
    
    container.innerHTML = publicGoals.map(goal => {
        const displayedName = getDisplayedName(goal);
        const interactions = socialInteractions[goal.id] || { likes: [], dislikes: [], comments: [] };
        const userLiked = interactions.likes.includes('current-user');
        const userDisliked = interactions.dislikes.includes('current-user');
        
        return `
            <div class="goal-item">
                <div class="goal-header">
                    <div>
                        <div class="goal-title">${goal.title}</div>
                        <span class="goal-type">${goal.type}</span>
                    </div>
                </div>
                
                ${goal.description ? `<div class="goal-description">${goal.description}</div>` : ''}
                
                <div class="progress-container">
                    <div class="progress-label">
                        <span>Progression</span>
                        <span>${goal.progress}%</span>
                    </div>
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: ${goal.progress}%"></div>
                    </div>
                </div>
                
                <div class="goal-meta">
                    <span><i class="fas fa-user"></i> ${displayedName}</span>
                    <span><i class="fas fa-clock"></i> Mis à jour le ${formatDate(goal.lastUpdate)}</span>
                </div>
                
                <div class="social-actions">
                    <button class="social-btn ${userLiked ? 'active' : ''}" onclick="toggleLike('${goal.id}')">
                        <i class="fas fa-thumbs-up"></i>
                        <span>${interactions.likes.length}</span>
                    </button>
                    <button class="social-btn dislike ${userDisliked ? 'active' : ''}" onclick="toggleDislike('${goal.id}')">
                        <i class="fas fa-thumbs-down"></i>
                        <span>${interactions.dislikes.length}</span>
                    </button>
                    ${goal.allowComments ? `
                        <button class="social-btn" onclick="toggleComments('${goal.id}')">
                            <i class="fas fa-comment"></i>
                            <span>${interactions.comments.length}</span>
                        </button>
                    ` : ''}
                </div>
                
                ${goal.allowComments ? `
                    <div class="comments-section" id="comments-${goal.id}" style="display: none;">
                        <div class="comment-form">
                            <input type="text" placeholder="Écrire un commentaire..." id="comment-input-${goal.id}">
                            <button class="btn" onclick="addComment('${goal.id}')">
                                <i class="fas fa-paper-plane"></i>
                            </button>
                        </div>
                        <div id="comments-list-${goal.id}">
                            ${interactions.comments.map(c => `
                                <div class="comment-item">
                                    <div class="comment-author">${c.author}</div>
                                    <div>${c.text}</div>
                                </div>
                            `).join('')}
                        </div>
                    </div>
                ` : ''}
            </div>
        `;
    }).join('');
}

// ========================================
// INTERACTIONS SOCIALES
// ========================================

function toggleLike(goalId) {
    if (!socialInteractions[goalId]) {
        socialInteractions[goalId] = { likes: [], dislikes: [], comments: [] };
    }
    
    const likes = socialInteractions[goalId].likes;
    const dislikes = socialInteractions[goalId].dislikes;
    const userIndex = likes.indexOf('current-user');
    
    if (userIndex > -1) {
        likes.splice(userIndex, 1);
    } else {
        likes.push('current-user');
        const dislikeIndex = dislikes.indexOf('current-user');
        if (dislikeIndex > -1) dislikes.splice(dislikeIndex, 1);
    }
    
    saveSocialInteractions();
    renderPublicGoals();
}

function toggleDislike(goalId) {
    if (!socialInteractions[goalId]) {
        socialInteractions[goalId] = { likes: [], dislikes: [], comments: [] };
    }
    
    const likes = socialInteractions[goalId].likes;
    const dislikes = socialInteractions[goalId].dislikes;
    const userIndex = dislikes.indexOf('current-user');
    
    if (userIndex > -1) {
        dislikes.splice(userIndex, 1);
    } else {
        dislikes.push('current-user');
        const likeIndex = likes.indexOf('current-user');
        if (likeIndex > -1) likes.splice(likeIndex, 1);
    }
    
    saveSocialInteractions();
    renderPublicGoals();
}

function toggleComments(goalId) {
    const commentsSection = document.getElementById(`comments-${goalId}`);
    commentsSection.style.display = commentsSection.style.display === 'none' ? 'block' : 'none';
}

function addComment(goalId) {
    const input = document.getElementById(`comment-input-${goalId}`);
    const text = input.value.trim();
    
    if (!text) return;
    
    if (!socialInteractions[goalId]) {
        socialInteractions[goalId] = { likes: [], dislikes: [], comments: [] };
    }
    
    socialInteractions[goalId].comments.push({
        author: 'Utilisateur',
        text: text,
        date: new Date().toISOString()
    });
    
    saveSocialInteractions();
    input.value = '';
    renderPublicGoals();
    toggleComments(goalId);
}

// ========================================
// CHECK-IN QUOTIDIEN
// ========================================

function renderCheckIn() {
    const select = document.getElementById('checkin-goal-select');
    select.innerHTML = '<option value="">-- Sélectionner un objectif --</option>' + 
        goals.map(g => `<option value="${g.id}">${g.title}</option>`).join('');
    
    select.addEventListener('change', (e) => {
        const goal = goals.find(g => g.id === e.target.value);
        if (goal) {
            document.getElementById('checkin-progress').value = goal.progress;
            checkinProgressValue.textContent = goal.progress + '%';
        }
    });
    
    renderCheckInHistory();
}

document.getElementById('btn-save-checkin').addEventListener('click', () => {
    const goalId = document.getElementById('checkin-goal-select').value;
    const note = document.getElementById('checkin-note').value.trim();
    const progress = parseInt(document.getElementById('checkin-progress').value);
    
    if (!goalId) {
        alert('Veuillez sélectionner un objectif');
        return;
    }
    
    if (!note) {
        alert('Veuillez écrire une note');
        return;
    }
    
    // Mettre à jour la progression de l'objectif
    const goal = goals.find(g => g.id === goalId);
    if (goal) {
        goal.progress = progress;
        goal.lastUpdate = new Date().toISOString();
        saveGoals();
    }
    
    // Enregistrer le check-in
    const checkin = {
        id: Date.now().toString(),
        goalId: goalId,
        goalTitle: goal.title,
        note: note,
        progress: progress,
        date: new Date().toISOString()
    };
    
    checkins.push(checkin);
    saveCheckIns();
    
    // Réinitialiser le formulaire
    document.getElementById('checkin-note').value = '';
    
    alert('Check-in enregistré avec succès !');
    renderCheckInHistory();
});

function renderCheckInHistory() {
    const container = document.getElementById('checkin-history');
    
    if (checkins.length === 0) {
        container.innerHTML = `
            <div class="card">
                <div class="empty-state">
                    <i class="fas fa-clipboard-list"></i>
                    <p>Aucun check-in enregistré</p>
                </div>
            </div>
        `;
        return;
    }
    
    const sortedCheckins = [...checkins].sort((a, b) => new Date(b.date) - new Date(a.date));
    
    container.innerHTML = `
        <div class="card">
            <h3 style="margin-bottom: 20px;">Historique des check-ins</h3>
            ${sortedCheckins.map(c => `
                <div class="checkin-item">
                    <div class="checkin-date">${formatDate(c.date)}</div>
                    <div class="checkin-progress">
                        <strong>${c.goalTitle}</strong> - ${c.progress}%
                    </div>
                    <div class="checkin-note">${c.note}</div>
                </div>
            `).join('')}
        </div>
    `;
}

// ========================================
// DÉTAILS OBJECTIF
// ========================================

function viewGoalDetails(goalId) {
    const goal = goals.find(g => g.id === goalId);
    if (!goal) return;
    
    const goalCheckins = checkins.filter(c => c.goalId === goalId);
    
    const content = `
        <div>
            <h3 style="margin-bottom: 16px;">${goal.title}</h3>
            <div class="goal-meta" style="margin-bottom: 20px;">
                <span><i class="fas fa-tag"></i> ${goal.type}</span>
                <span><i class="fas fa-eye${goal.visibility === 'privé' ? '-slash' : ''}"></i> ${goal.visibility}</span>
            </div>
            
            ${goal.description ? `<p style="margin-bottom: 20px; color: var(--text-light);">${goal.description}</p>` : ''}
            
            <div class="progress-container">
                <div class="progress-label">
                    <span>Progression actuelle</span>
                    <span>${goal.progress}%</span>
                </div>
                <div class="progress-bar">
                    <div class="progress-fill" style="width: ${goal.progress}%"></div>
                </div>
            </div>
            
            <div style="margin-top: 24px;">
                <h4 style="margin-bottom: 12px;">Check-ins (${goalCheckins.length})</h4>
                ${goalCheckins.length === 0 ? '<p style="color: var(--text-light);">Aucun check-in pour cet objectif</p>' : 
                    goalCheckins.sort((a, b) => new Date(b.date) - new Date(a.date)).map(c => `
                        <div class="checkin-item">
                            <div class="checkin-date">${formatDate(c.date)}</div>
                            <div class="checkin-progress">${c.progress}%</div>
                            <div class="checkin-note">${c.note}</div>
                        </div>
                    `).join('')
                }
            </div>
        </div>
    `;
    
    document.getElementById('goal-details-content').innerHTML = content;
    modalDetails.classList.add('active');
}

// ========================================
// SUPPRESSION OBJECTIF
// ========================================

function deleteGoal(goalId) {
    if (!confirm('Êtes-vous sûr de vouloir supprimer cet objectif ?')) return;
    
    goals = goals.filter(g => g.id !== goalId);
    checkins = checkins.filter(c => c.goalId !== goalId);
    delete socialInteractions[goalId];
    
    saveGoals();
    saveCheckIns();
    saveSocialInteractions();
    renderMyGoals();
}

// ========================================
// UTILITAIRES
// ========================================

function getDisplayedName(goal) {
    if (goal.displayName === 'anonyme') return 'Anonyme';
    if (goal.displayName === 'pseudo') return goal.userName || 'Utilisateur';
    if (goal.displayName === 'nom') return goal.userName || 'Utilisateur';
    return 'Utilisateur';
}

function formatDate(dateString) {
    const date = new Date(dateString);
    return date.toLocaleDateString('fr-FR', { 
        day: '2-digit', 
        month: '2-digit', 
        year: 'numeric',
        hour: '2-digit',
        minute: '2-digit'
    });
}

function saveGoals() {
    localStorage.setItem('goals', JSON.stringify(goals));
}

function saveCheckIns() {
    localStorage.setItem('checkins', JSON.stringify(checkins));
}

function saveSocialInteractions() {
    localStorage.setItem('socialInteractions', JSON.stringify(socialInteractions));
}

// ========================================
// INITIALISATION
// ========================================

renderMyGoals();
checkinProgressSlider.addEventListener('input', (e) => {
    checkinProgressValue.textContent = e.target.value + '%';
});

// ========================================
// CRÉER/MODIFIER OBJECTIF
// ========================================

function openGoalModal(goalId = null) {
    const form = document.getElementById('goal-form');
    form.reset();

    if (goalId) {
        const goal = goals.find(g => g.id === goalId);
        if (goal) {
            document.getElementById('modal-goal-title').textContent = 'Modifier l\'objectif';
            document.getElementById('goal-id').value = goal.id;
            document.getElementById('goal-title').value = goal.title;
            document.getElementById('goal-description').value = goal.description;
            document.querySelector(`input[name="goal-type"][value="${goal.type}"]`).checked = true;
            document.getElementById('goal-progress').value = goal.progress;
            goalProgressValue.textContent = goal.progress + '%';
            document.querySelector(`input[name="goal-visibility"][value="${goal.visibility}"]`).checked = true;
            document.querySelector(`input[name="goal-display-name"][value="${goal.displayName}"]`).checked = true;
            document.getElementById('goal-user-name').value = goal.userName || '';
            document.getElementById('goal-allow-comments').checked = goal.allowComments;
        }
    } else {
        document.getElementById('modal-goal-title').textContent = 'Créer un objectif';
        goalProgressValue.textContent = '0%';
    }

    modalGoal.classList.add('active');
}

document.getElementById('goal-form').addEventListener('submit', (e) => {
    e.preventDefault();

    const goalId = document.getElementById('goal-id').value;
    const title = document.getElementById('goal-title').value;
    const description = document.getElementById('goal-description').value;
    const type = document.querySelector('input[name="goal-type"]:checked').value;
    const progress = parseInt(document.getElementById('goal-progress').value);
    const visibility = document.querySelector('input[name="goal-visibility"]:checked').value;
    const displayName = document.querySelector('input[name="goal-display-name"]:checked').value;
    const userName = document.getElementById('goal-user-name').value;
    const allowComments = document.getElementById('goal-allow-comments').checked;

    if (goalId) {
        // Modifier
        const index = goals.findIndex(g => g.id === goalId);
        if (index !== -1) {
            goals[index] = {
                ...goals[index],
                title,
                description,
                type,
                progress,
                visibility,
                displayName,
                userName,
                allowComments,
                lastUpdate: new Date().toISOString()
            };
        }
    } else {
        // Créer
        const newGoal = {
            id: Date.now().toString(),
            title,
            description,
            type,
            progress,
            visibility,
            displayName,
            userName,
            allowComments,
            createdAt: new Date().toISOString(),
            lastUpdate: new Date().toISOString()
        };
        goals.push(newGoal);
    }

    saveGoals();
    modalGoal.classList.remove('active');
    renderMyGoals();
});

// ========================================
// AFFICHAGE MES OBJECTIFS
// ========================================

function renderMyGoals() {
    const container = document.getElementById('my-goals-list');

    if (goals.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <i class="fas fa-bullseye"></i>
                <p>Aucun objectif créé</p>
                <p style="font-size: 14px; margin-top: 8px;">Cliquez sur "Créer un objectif" pour commencer</p>
            </div>
        `;
        return;
    }

    container.innerHTML = goals.map(goal => `
        <div class="goal-item">
            <div class="goal-header">
                <div>
                    <div class="goal-title">${goal.title}</div>
                    <span class="goal-type">${goal.type}</span>
                </div>
            </div>

            ${goal.description ? `<div class="goal-description">${goal.description}</div>` : ''}

            <div class="progress-container">
                <div class="progress-label">
                    <span>Progression</span>
                    <span>${goal.progress}%</span>
                </div>
                <div class="progress-bar">
                    <div class="progress-fill" style="width: ${goal.progress}%"></div>
                </div>
            </div>

            <div class="goal-meta">
                <span><i class="fas fa-eye${goal.visibility === 'privé' ? '-slash' : ''}"></i> ${goal.visibility === 'privé' ? 'Privé' : 'Public'}</span>
                <span><i class="fas fa-calendar"></i> Créé le ${formatDate(goal.createdAt)}</span>
                <span><i class="fas fa-clock"></i> Mis à jour le ${formatDate(goal.lastUpdate)}</span>
            </div>

            <div class="goal-actions">
                <button class="btn btn-secondary" onclick="openGoalModal('${goal.id}')">
                    <i class="fas fa-edit"></i> Modifier
                </button>
                <button class="btn btn-secondary" onclick="viewGoalDetails('${goal.id}')">
                    <i class="fas fa-info-circle"></i> Détails
                </button>
                <button class="btn btn-danger" onclick="deleteGoal('${goal.id}')">
                    <i class="fas fa-trash"></i> Supprimer
                </button>
                ${goal.progress === 100 ? '<button class="btn btn-success"><i class="fas fa-check"></i> Terminé</button>' : ''}
            </div>
        </div>
    `).join('');
}

// ========================================
// AFFICHAGE OBJECTIFS PUBLICS
// ========================================

function renderPublicGoals() {
    const container = document.getElementById('public-goals-list');
    const publicGoals = goals.filter(g => g.visibility === 'public');

    if (publicGoals.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <i class="fas fa-globe"></i>
                <p>Aucun objectif public</p>
                <p style="font-size: 14px; margin-top: 8px;">Les objectifs publics apparaîtront ici</p>
            </div>
        `;
        return;
    }

    container.innerHTML = publicGoals.map(goal => {
        const displayedName = getDisplayedName(goal);
        const interactions = socialInteractions[goal.id] || { likes: [], dislikes: [], comments: [] };
        const userLiked = interactions.likes.includes('current-user');
        const userDisliked = interactions.dislikes.includes('current-user');

        return `
            <div class="goal-item">
                <div class="goal-header">
                    <div>
                        <div class="goal-title">${goal.title}</div>
                        <span class="goal-type">${goal.type}</span>
                    </div>
                </div>

                ${goal.description ? `<div class="goal-description">${goal.description}</div>` : ''}

                <div class="progress-container">
                    <div class="progress-label">
                        <span>Progression</span>
                        <span>${goal.progress}%</span>
                    </div>
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: ${goal.progress}%"></div>
                    </div>
                </div>

                <div class="goal-meta">
                    <span><i class="fas fa-user"></i> ${displayedName}</span>
                    <span><i class="fas fa-clock"></i> Mis à jour le ${formatDate(goal.lastUpdate)}</span>
                </div>

                <div class="social-actions">
                    <button class="social-btn ${userLiked ? 'active' : ''}" onclick="toggleLike('${goal.id}')">
                        <i class="fas fa-thumbs-up"></i>
                        <span>${interactions.likes.length}</span>
                    </button>
                    <button class="social-btn dislike ${userDisliked ? 'active' : ''}" onclick="toggleDislike('${goal.id}')">
                        <i class="fas fa-thumbs-down"></i>
                        <span>${interactions.dislikes.length}</span>
                    </button>
                    ${goal.allowComments ? `
                        <button class="social-btn" onclick="toggleComments('${goal.id}')">
                            <i class="fas fa-comment"></i>
                            <span>${interactions.comments.length}</span>
                        </button>
                    ` : ''}
                </div>

                ${goal.allowComments ? `
                    <div class="comments-section" id="comments-${goal.id}" style="display: none;">
                        <div class="comment-form">
                            <input type="text" placeholder="Écrire un commentaire..." id="comment-input-${goal.id}">
                            <button class="btn" onclick="addComment('${goal.id}')">
                                <i class="fas fa-paper-plane"></i>
                            </button>
                        </div>
                        <div id="comments-list-${goal.id}">
                            ${interactions.comments.map(c => `
                                <div class="comment-item">
                                    <div class="comment-author">${c.author}</div>
                                    <div>${c.text}</div>
                                </div>
                            `).join('')}
                        </div>
                    </div>
                ` : ''}
            </div>
        `;
    }).join('');
}

// ========================================
// INTERACTIONS SOCIALES
// ========================================

function toggleLike(goalId) {
    if (!socialInteractions[goalId]) {
        socialInteractions[goalId] = { likes: [], dislikes: [], comments: [] };
    }

    const likes = socialInteractions[goalId].likes;
    const dislikes = socialInteractions[goalId].dislikes;
    const userIndex = likes.indexOf('current-user');

    if (userIndex > -1) {
        likes.splice(userIndex, 1);
    } else {
        likes.push('current-user');
        const dislikeIndex = dislikes.indexOf('current-user');
        if (dislikeIndex > -1) dislikes.splice(dislikeIndex, 1);
    }

    saveSocialInteractions();
    renderPublicGoals();
}

function toggleDislike(goalId) {
    if (!socialInteractions[goalId]) {
        socialInteractions[goalId] = { likes: [], dislikes: [], comments: [] };
    }

    const likes = socialInteractions[goalId].likes;
    const dislikes = socialInteractions[goalId].dislikes;
    const userIndex = dislikes.indexOf('current-user');

    if (userIndex > -1) {
        dislikes.splice(userIndex, 1);
    } else {
        dislikes.push('current-user');
        const likeIndex = likes.indexOf('current-user');
        if (likeIndex > -1) likes.splice(likeIndex, 1);
    }

    saveSocialInteractions();
    renderPublicGoals();
}

function toggleComments(goalId) {
    const commentsSection = document.getElementById(`comments-${goalId}`);
    commentsSection.style.display = commentsSection.style.display === 'none' ? 'block' : 'none';
}

function addComment(goalId) {
    const input = document.getElementById(`comment-input-${goalId}`);
    const text = input.value.trim();

    if (!text) return;

    if (!socialInteractions[goalId]) {
        socialInteractions[goalId] = { likes: [], dislikes: [], comments: [] };
    }

    socialInteractions[goalId].comments.push({
        author: 'Utilisateur',
        text: text,
        date: new Date().toISOString()
    });

    saveSocialInteractions();
    input.value = '';
    renderPublicGoals();
    toggleComments(goalId);
}

// ========================================
// CHECK-IN QUOTIDIEN
// ========================================

function renderCheckIn() {
    const select = document.getElementById('checkin-goal-select');
    select.innerHTML = '<option value="">-- Sélectionner un objectif --</option>' + 
        goals.map(g => `<option value="${g.id}">${g.title}</option>`).join('');

    select.addEventListener('change', (e) => {
        const goal = goals.find(g => g.id === e.target.value);
        if (goal) {
            document.getElementById('checkin-progress').value = goal.progress;
            checkinProgressValue.textContent = goal.progress + '%';
        }
    });

    renderCheckInHistory();
}

document.getElementById('btn-save-checkin').addEventListener('click', () => {
    const goalId = document.getElementById('checkin-goal-select').value;
    const note = document.getElementById('checkin-note').value.trim();
    const progress = parseInt(document.getElementById('checkin-progress').value);

    if (!goalId) {
        alert('Veuillez sélectionner un objectif');
        return;
    }

    if (!note) {
        alert('Veuillez écrire une note');
        return;
    }

    const goal = goals.find(g => g.id === goalId);
    if (goal) {
        goal.progress = progress;
        goal.lastUpdate = new Date().toISOString();
        saveGoals();
    }

    const checkin = {
        id: Date.now().toString(),
        goalId: goalId,
        goalTitle: goal.title,
        note: note,
        progress: progress,
        date: new Date().toISOString()
    };

    checkins.push(checkin);
    saveCheckIns();

    document.getElementById('checkin-note').value = '';

    alert('Check-in enregistré avec succès !');
    renderCheckInHistory();
});

function renderCheckInHistory() {
    const container = document.getElementById('checkin-history');

    if (checkins.length === 0) {
        container.innerHTML = `
            <div class="card">
                <div class="empty-state">
                    <i class="fas fa-clipboard-list"></i>
                    <p>Aucun check-in enregistré</p>
                </div>
            </div>
        `;
        return;
    }

    const sortedCheckins = [...checkins].sort((a, b) => new Date(b.date) - new Date(a.date));

    container.innerHTML = `
        <div class="card">
            <h3 style="margin-bottom: 20px;">Historique des check-ins</h3>
            ${sortedCheckins.map(c => `
                <div class="checkin-item">
                    <div class="checkin-date">${formatDate(c.date)}</div>
                    <div class="checkin-progress">
                        <strong>${c.goalTitle}</strong> - ${c.progress}%
                    </div>
                    <div class="checkin-note">${c.note}</div>
                </div>
            `).join('')}
        </div>
    `;
}

// ========================================
// DÉTAILS OBJECTIF
// ========================================

function viewGoalDetails(goalId) {
    const goal = goals.find(g => g.id === goalId);
    if (!goal) return;

    const goalCheckins = checkins.filter(c => c.goalId === goalId);

    const content = `
        <div>
            <h3 style="margin-bottom: 16px;">${goal.title}</h3>
            <div class="goal-meta" style="margin-bottom: 20px;">
                <span><i class="fas fa-tag"></i> ${goal.type}</span>
                <span><i class="fas fa-eye${goal.visibility === 'privé' ? '-slash' : ''}"></i> ${goal.visibility}</span>
            </div>

            ${goal.description ? `<p style="margin-bottom: 20px; color: var(--text-light);">${goal.description}</p>` : ''}

            <div class="progress-container">
                <div class="progress-label">
                    <span>Progression actuelle</span>
                    <span>${goal.progress}%</span>
                </div>
                <div class="progress-bar">
                    <div class="progress-fill" style="width: ${goal.progress}%"></div>
                </div>
            </div>

            <div style="margin-top: 24px;">
                <h4 style="margin-bottom: 12px;">Check-ins (${goalCheckins.length})</h4>
                ${goalCheckins.length === 0 ? '<p style="color: var(--text-light);">Aucun check-in pour cet objectif</p>' : 
                    goalCheckins.sort((a, b) => new Date(b.date) - new Date(a.date)).map(c => `
                        <div class="checkin-item">
                            <div class="checkin-date">${formatDate(c.date)}</div>
                            <div class="checkin-progress">${c.progress}%</div>
                            <div class="checkin-note">${c.note}</div>
                       
